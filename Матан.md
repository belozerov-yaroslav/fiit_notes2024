...
$D, \,\,\gamma=\partial D, P(x, y), Q(x, y)$ - непрерывно дифференцируемы

$\tau = (dx, dy)$
$a = (P, Q)$
$\int\int_D(Q^{'}_x-P^{'}_y)dxdy = \int_{\gamma+}P dx+Q dy = \int_{\gamma+}(a, \tau)ds$

#### Доказательство

Пусть $D = \bigcup_{k=1}A_k, (A_k \cap^{\bigcirc} A_j) = \emptyset$

Случай 2) $A = \{(x,y): a <= x <= b, c <= y <= f(x)\},$ где $f(x)$ - строго монотонна и дифференцируема

Докажем для области типа 2:

$$\iint_A P_y^{'}(x ,y)dxdy = \int_a^b\int_c^{f(x)}-P_y^{'}dydx = \int_a^b-P(x, y)|_c^{f(x)}dx = \int_b^a(P(x, c) - P(x, f(x)))dx=$$
$$=\int_a^bP(x, c)dx + \int0dx - \int_a^bP(x, f(x))dx = $$
$$\int_{I_1}^aP(x, y)+\int_{I_2}P(x, y)dx+\int_{I_3}P(x,y)dx = \oint_{\gamma+}P(x,y)dx$$
___
$$\iint_DQ_x^{'}dxdy = \int_c^d\int_{f^{-1}(y)}^b Q^{'}_x dxdy$ = \int_c^d(Q(b, y)-Q(f^{-1}(y), y))dy = $$
$$\int_{I_2}Qdy + \int_{I_3}Qdy + \int_{I_1}Qdy = \oint_\gamma Qdy$$
#### Следствие

Если $P^{'}_y = Q_x^{'}$, то интеграл не зависит от пути и
$\oint_I Pdx + Qdy = \int_{I_2}Pdx+Qdy$

##### Доказательство следствия

$$\int_\gamma Pdx + Qdy = \int_{I_1}Pdx + Qdy + \int_{I_2}Pdx+Qdy = 0$$
$$\int_{I_1}Pdx + Qdy = \int_{I_2+}Pdx + Qdy$$
...
будем называть гладкой поверхностью, заданной явно, если $f(x, y)$ непрерывно дифференцируема и $S: (x, y) \rightarrow (x, y, f(x, y))$

### Определение

Площадью гладкой поверхности $M(S) = \iint_{D-} \sqrt{(f^{'2}_x + f^{'2}_y}dxdy$

#### Следствие

Если поверхность $S \,\, (p, q) \rightarrow (x, y, u)$ задана параметрически, то 
$$M(S) = \iint_{D+}\sqrt{(\delta xy)^2 + (\delta xu)^2 + (\delta yu)^2}dpdq$$
