...
Если положить $dx_k=\Delta x_k=x_k-x_k^0,$ то
### Теорема (формула Тейлора в форме Лагранжа)
...
 $$f(\mathbf{x})=f(\mathbf{x}^0)+\sum_{k=1}^m f^\prime_{x_k}(\mathbf{x}^0)\Delta x_k+\frac{1}{2}\sum_{k=1}^m\sum_{j=1}^m f^{\prime\prime}_{x_kx_j}(\mathbf{x}^0)\Delta x_k\Delta x_j+o(\|\Delta \mathbf{x}\|^2).$$
 $$f(\mathbf{x})-f(\mathbf{x}^0)=\frac{\|\Delta x\|^2}{2}\left(\sum_{k=1}^m\sum_{j=1}^m f^{\prime\prime}_{x_kx_j}(\mathbf{x}^0)\frac{\Delta x_k}{\|\Delta \mathbf{x}\|}\frac{\Delta x_j}{\|\Delta \mathbf{x}\|}+o(1)\right)$$
...
По теореме Вейерштрасса $\exists a^0\in S(0,1): \inf_{a\in S(0,1)} \Phi(\mathbf{a})=\Phi(\mathbf{a}^0)>0.$ \pause
